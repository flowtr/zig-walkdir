    ...
    pub fn next(self: *Self) !?Entry {
        outer: while (true) {
            if (try self.current_iter.next()) |entry| {
                ...
                const full_entry_path = try self.allocator.alloc(
                    u8,
                    self.current_path.len + entry.name.len + 1,
                );
                std.mem.copy(u8, full_entry_path, self.current_path);
                full_entry_path[self.current_path.len] = std.fs.path.sep;
                std.mem.copy(
                    u8,
                    full_entry_path[self.current_path.len + 1 ..],
                    entry.name,
                );
                ...
                blk: {
                    if (entry.kind == std.fs.IterableDir.Entry.Kind.Directory) {
                        ...
                        // Go one level deeper
                        var opened_dir = try std.fs.cwd().openIterableDir(
                            full_entry_path,
                            .{},
                        );
                        self.current_path = try self.allocator.dupe(
                            u8,
                            full_entry_path,
                        );
                        self.current_dir = opened_dir.dir;
